...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " <password_length>\n";
        return 1;
    }

    int length = std::stoi(argv[1]);
    if (length < 1 || length > 10) {
        std::cout << "Please use password length between 1 and 10 for practicality.\n";
        return 1;
    }
    ...
    std::cout << "MD5 hash of \"" << passCode << "\" is: " << target << "\n";
    ...
    for (uint64_t i = 0; i < totalCombos; ++i) {
        uint64_t temp = i;
        ...
        if (md5_hash(attempt) == target) {
            std::cout << "Found password: " << attempt << "\n";
            return 0;
        }
    }
